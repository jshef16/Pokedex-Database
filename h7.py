...
def setUpPokemonTable(data, cur, conn):
    ...
    for row in data:
        cur.execute("SELECT id FROM Types WHERE type = ?", (row['type'][0],))
        poke_type = cur.fetchone()[0]
        cur.execute("INSERT INTO Pokemon (name, type_id, HP, attack, defense, speed) VALUES (?, ?, ?, ?, ?, ?)", (row['name']['english'], poke_type, row['base']['HP'], row['base']['Attack'], row['base']['Defense'], row['base']['Speed']))
        conn.commit()
...
def getPokemonAboveSpeedAboveDefenseOfType(speed, defense, type, cur, conn):
    cur.execute("SELECT Pokemon.name, Types.type, Pokemon.speed, Pokemon.defense FROM Pokemon JOIN Types ON Pokemon.type_id = Types.id WHERE Types.type = ? AND speed > ? AND defense > ?", (type, speed, defense))
    ans = cur.fetchall()
    return ans
...
def getPokemonOfType(type, attack, defense, cur, conn):
    cur.execute("SELECT Pokemon.name FROM Pokemon JOIN Types ON Pokemon.type_id = Types.id WHERE Types.type = ? AND attack >= ? AND defense = ?", (type, attack, defense))
    ans = cur.fetchall()
    return ans
